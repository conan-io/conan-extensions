...
def test_build_info_create_no_deps():
    ...
    run(f'conan art:build-info append {build_name}_aggregated {build_number} {os.getenv("ART_URL")} --build-info={build_name}_release,{build_number} --build-info={build_name}_debug,{build_number} > {build_name}_aggregated.json')
